...
function SearchResult() {
    ...
    const transformedResults = domainResults.flatMap((res) => 
        res.trainingDomain.map((domain) => ({
            ...res,
                                ...
                                <div key={_i} className='border flex items-start justify-between border-gray-200 rounded-md p-3 h-max w-[70vw]'>
                                    ...
                                    <div className='flex items-start flex-col justify-between'>
                                        <div className='flex items-center'>
                                            ...
                                            <p className='ml-2 font-medium'>{res.generalDetails?.name} - 
                                    ...
                                    <div className='flex items-center justify-between'>
                                        {res.trainingDomain?.paymentSession &&
                                            <p className='flex items-center'>
                                                <ion-icon style={{ fontSize: "20px", color: "gray", marginRight: "5px" }} name="calendar-outline"></ion-icon>
                                                <span className='font-medium'>{res.trainingDomain?.paymentSession}</span>
                                            </p>
                                        }
                                        <div className='ml-4'>
                                            <h2 className='text-gray-600'>Price</h2>
                                            <p>
                                                <span className='text-black font-medium'>₹{res.trainingDomain?.price}</span>
                                            </p>
                                        </div>
                                    </div>
