...
function ResumeDownload() {
    ...
    return ( <
        ...
        hr className = 'pb-8' / >
        <
        div className = 'h-full' > { /* New template */ } <
        div className = "bg-white w-[90vw] lg:w-[80vw]  h-max relative"
        ref = { resumeRef }
        id = "resumeRef" >
        ...
        img src = { ResumeLogo }
        alt = "Logo"
        className = "mb-8  w-[40vw]" / >
        <
        /div> <
        /div> <
        div className = "" >
        <
        h1 className = "text-3xl font-bold text-resumeText mb-10" > {
            user
            .generalDetails
            .name
            .toUpperCase()
        ...
        { /* Resume content */ } <
        div className = "grid grid-cols-1 md:grid-cols-2 gap-10 mt-4" > { /* Left Column */ } <
        div className = "space-y-10" > {
            leftColumnContent.map((section, index) => ( <
                div key = { index } >
                <
                h2 className = "text-resumeText text-2xl font-semibold mb-2 border-y border-resumeLine py-1 px-4" > { section.title } <
                /h2> <
                ul className = "list-disc pl-5 space-y-2" > {
                    section.content.map((item, _i) => ( <
                        li className = "text-justify text-base leading-7"
                        key = { _i } > { item } <
                        /li>
                    ))
                } <
