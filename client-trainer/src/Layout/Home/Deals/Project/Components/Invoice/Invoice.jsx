...
function Invoice({ purchaseOrder, projectName, inVoice }) {
    ...
    return ( <
        div className = 'w-[90vw] lg:w-[80vw]  mt-8 p-6 bg-white rounded-md shadow-sm' > { /* Upload Invoice */ }
        ...
        /div> {
            purchaseOrder && purchaseOrder ? .details ? .description.length > 0 ?
                ...
                Fragment > {
                    ...
                    Button onClick = {
                        () => setShowInvoice(!showInvoice) }
                    className = "rounded-none" > Generate < /Button> <
                    /div> <
                    /div>
                } {
                    inVoice && inVoice.InvoiceUrl &&
            ...
            <
            /Fragment>: < p className = 'text-sm italic' > PO not issued yet < /p> 
        }
...

        {
            showInvoice && < div className = 'mt-4' >
                <
                GenerateInvoiceForm inVoice = { inVoice }
